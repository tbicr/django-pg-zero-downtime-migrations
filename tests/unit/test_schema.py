...
if django.VERSION[:2] >= (5, 0):
    ...
    class ModelWithGeneratedStored(models.Model):
        ...
        field = models.GeneratedField(
            expression=models.F('field1') + models.F('field2'),
            output_field=models.IntegerField(),
            db_persist=True,
        )

    class ModelWithGeneratedVirtual(models.Model):
        field1 = models.IntegerField()
        field2 = models.IntegerField()
        field = models.GeneratedField(
            expression=models.F('field1') + models.F('field2'),
...
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_create_model_with_generated_stored__ok():
    with cmp_schema_editor() as editor:
        editor.create_model(ModelWithGeneratedStored)
    assert editor.collected_sql == editor.django_sql
    assert editor.django_sql == [
        'CREATE TABLE "tests_modelwithgeneratedstored" '
        '("id" bigint NOT NULL PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, '
        '"field1" integer NOT NULL, "field2" integer NOT NULL, '
        '"field" integer GENERATED ALWAYS AS (("field1" + "field2")) STORED);',
    ]
...
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_create_model_with_generated_virtual__ok():
    with cmp_schema_editor() as editor:
        editor.create_model(ModelWithGeneratedVirtual)
    assert editor.collected_sql == editor.django_sql
    assert editor.django_sql == [
        'CREATE TABLE "tests_modelwithgeneratedvirtual" '
        '("id" bigint NOT NULL PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, '
        '"field1" integer NOT NULL, "field2" integer NOT NULL, '
        '"field" integer GENERATED ALWAYS AS (("field1" + "field2")) VIRTUAL);',
    ]
...
@pytest.mark.django_db
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
def test_add_field_with_generated_stored__warning():
    with cmp_schema_editor() as editor:
        with pytest.warns(UnsafeOperationWarning, match='ADD COLUMN GENERATED STORED is unsafe operation'):
            editor.add_field(ModelWithGeneratedStored, ModelWithGeneratedStored._meta.get_field('field'))
    assert editor.collected_sql == timeouts(editor.django_sql)
    assert editor.django_sql == [
        'ALTER TABLE "tests_modelwithgeneratedstored" ADD COLUMN "field" integer '
        'GENERATED ALWAYS AS (("field1" + "field2")) STORED;',
    ]
...
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_FLEXIBLE_STATEMENT_TIMEOUT=True)
def test_add_field_with_generated_stored__with_flexible_timeout__warning():
    with cmp_schema_editor() as editor:
        with pytest.warns(UnsafeOperationWarning, match='ADD COLUMN GENERATED STORED is unsafe operation'):
            editor.add_field(ModelWithGeneratedStored, ModelWithGeneratedStored._meta.get_field('field'))
    assert editor.collected_sql == timeouts(editor.django_sql)
...
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_add_field_with_generated_stored__raise():
    with cmp_schema_editor() as editor:
        with pytest.raises(UnsafeOperationException, match='ADD COLUMN GENERATED STORED is unsafe operation'):
            editor.add_field(ModelWithGeneratedStored, ModelWithGeneratedStored._meta.get_field('field'))
    assert editor.django_sql == [
        'ALTER TABLE "tests_modelwithgeneratedstored" ADD COLUMN "field" integer '
        'GENERATED ALWAYS AS (("field1" + "field2")) STORED;',
...
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_add_field_with_generated_virtual__ok():
    with cmp_schema_editor() as editor:
        editor.add_field(ModelWithGeneratedVirtual, ModelWithGeneratedVirtual._meta.get_field('field'))
    assert editor.collected_sql == timeouts(editor.django_sql)
    assert editor.django_sql == [
        'ALTER TABLE "tests_modelwithgeneratedvirtual" ADD COLUMN "field" integer '
        'GENERATED ALWAYS AS (("field1" + "field2")) VIRTUAL;',
    ]
...
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_alter_field_generic_to_generated_stored__not_supported():
    with cmp_schema_editor() as editor:
        old_field = models.IntegerField(null=True)
        old_field.set_attributes_from_name('field')
        with pytest.raises(ValueError, match='Modifying GeneratedFields is not supported'):
            editor.alter_field(
                ModelWithGeneratedStored, old_field, ModelWithGeneratedStored._meta.get_field('field'))


@pytest.mark.django_db
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_alter_field_generic_to_generated_virtual__not_supported():
    with cmp_schema_editor() as editor:
        old_field = models.IntegerField(null=True)
        old_field.set_attributes_from_name('field')
        with pytest.raises(ValueError, match='Modifying GeneratedFields is not supported'):
            editor.alter_field(
                ModelWithGeneratedVirtual, old_field, ModelWithGeneratedVirtual._meta.get_field('field'))


@pytest.mark.django_db
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_alter_field_generated_stored_to_generic__not_supported():
    with cmp_schema_editor() as editor:
        new_field = models.IntegerField(null=True)
        new_field.set_attributes_from_name('field')
        with pytest.raises(ValueError, match='Modifying GeneratedFields is not supported'):
            editor.alter_field(
                ModelWithGeneratedStored, ModelWithGeneratedStored._meta.get_field('field'), new_field)


@pytest.mark.django_db
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_alter_field_generated_virtual_to_generic__not_supported():
    with cmp_schema_editor() as editor:
        new_field = models.IntegerField(null=True)
        new_field.set_attributes_from_name('field')
        with pytest.raises(ValueError, match='Modifying GeneratedFields is not supported'):
            editor.alter_field(
                ModelWithGeneratedVirtual, ModelWithGeneratedVirtual._meta.get_field('field'), new_field)


@pytest.mark.django_db
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_alter_field_generated_stored_to_virtual__not_supported():
    with cmp_schema_editor() as editor:
        with pytest.raises(ValueError, match='Modifying GeneratedFields is not supported'):
            editor.alter_field(
                ModelWithGeneratedVirtual,
                ModelWithGeneratedStored._meta.get_field('field'),
                ModelWithGeneratedVirtual._meta.get_field('field'),
            )
...
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_alter_field_generated_virtual_to_stored__not_supported():
    with cmp_schema_editor() as editor:
        with pytest.raises(ValueError, match='Modifying GeneratedFields is not supported'):
            editor.alter_field(
                ModelWithGeneratedStored,
                ModelWithGeneratedVirtual._meta.get_field('field'),
                ModelWithGeneratedStored._meta.get_field('field'),
            )
...
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_remove_field_with_generated_stored__ok():
    with cmp_schema_editor() as editor:
        editor.remove_field(ModelWithGeneratedStored, ModelWithGeneratedStored._meta.get_field('field'))
    assert editor.collected_sql == timeouts(editor.django_sql)
    if django.VERSION[:2] >= (6, 0):
        assert editor.django_sql == [
            'ALTER TABLE "tests_modelwithgeneratedstored" DROP COLUMN "field";',
        ]
    else:
        assert editor.django_sql == [
            'ALTER TABLE "tests_modelwithgeneratedstored" DROP COLUMN "field" CASCADE;',
        ]
...
@pytest.mark.skipif(django.VERSION[:2] < (5, 0), reason='functionality provided in django 5.0')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_remove_field_with_generated_virtual__ok():
    with cmp_schema_editor() as editor:
        editor.remove_field(ModelWithGeneratedVirtual, ModelWithGeneratedVirtual._meta.get_field('field'))
    assert editor.collected_sql == timeouts(editor.django_sql)
    if django.VERSION[:2] >= (6, 0):
        assert editor.django_sql == [
            'ALTER TABLE "tests_modelwithgeneratedvirtual" DROP COLUMN "field";',
        ]
    else:
        assert editor.django_sql == [
            'ALTER TABLE "tests_modelwithgeneratedvirtual" DROP COLUMN "field" CASCADE;',
        ]
...
@pytest.mark.skipif(django.VERSION[:2] < (6, 1), reason='functionality provided in django 6.1')
@override_settings(ZERO_DOWNTIME_MIGRATIONS_RAISE_FOR_UNSAFE=True)
def test_create_model_with_unique_together_and_remove_it__deferred_sql__ok():
    with cmp_schema_editor() as editor:
        editor.create_model(ModelWithUniqueTogether)
        editor.alter_unique_together(ModelWithUniqueTogether, [['field1', 'field2']], [])
    # the unique statement is still deferred, thus django drops it instead of creating the constraint
    assert editor.collected_sql == editor.django_sql
    assert editor.django_sql == [
        'CREATE TABLE "tests_modelwithuniquetogether" '
        '("id" bigint NOT NULL PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, '
        '"field1" integer NOT NULL, "field2" integer NOT NULL);',
    ]
...
def test_create_model_with_unique_together_and_remove_it__no_deferred_sql__ok(mocker):
    mocker.patch.object(connection.introspection, 'get_constraints').return_value = {
        'tests_modelwithuniquetogether_field1_field2_b43f1549_uniq': {
            'columns': ['field1', 'field2'],
            'primary_key': False,
            'unique': True,
            'foreign_key': None,
    ...
    with cmp_schema_editor() as editor:
        editor.create_model(ModelWithUniqueTogether)
        editor.alter_unique_together(ModelWithUniqueTogether, [['field1', 'field2']], [])
    # the unique statement is already flushed, thus the constraint is created and dropped again,
    # while django keeps it deferred and drops the deferred statement
    assert editor.collected_sql == [
        'CREATE TABLE "tests_modelwithuniquetogether" '
        '("id" bigint NOT NULL PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, '
        '"field1" integer NOT NULL, "field2" integer NOT NULL);',
        'CREATE UNIQUE INDEX CONCURRENTLY "tests_modelwithuniquetogether_field1_field2_b43f1549_uniq" '
        'ON "tests_modelwithuniquetogether" ("field1", "field2");',
    ] + timeouts(
        'ALTER TABLE "tests_modelwithuniquetogether" '
        'ADD CONSTRAINT "tests_modelwithuniquetogether_field1_field2_b43f1549_uniq" '
        'UNIQUE USING INDEX "tests_modelwithuniquetogether_field1_field2_b43f1549_uniq";',
    ) + timeouts(
        'ALTER TABLE "tests_modelwithuniquetogether" '
        'DROP CONSTRAINT "tests_modelwithuniquetogether_field1_field2_b43f1549_uniq";',
    )
    assert editor.django_sql == [
        'CREATE TABLE "tests_modelwithuniquetogether" '
        '("id" bigint NOT NULL PRIMARY KEY GENERATED BY DEFAULT AS IDENTITY, '
